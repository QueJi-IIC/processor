...
folder_pth = r'E:\AIM & DREAM\Hackathons\IIC\Processor\Reverse_Lost_And_Foubd\Expected_Owners'
...
with mp_pose.Pose(min_detection_confidence=0.3, min_tracking_confidence=0.3) as pose:
    while True:
        ...
        results = ov_model.predict(frame, show=False, classes=[24, 26], conf=0.1, save=False)
        for r in results:
            num = len(r.boxes)
        ...
        if (num != 0):
            if results.pose_landmarks:
                ...
                if (current_dist >= threshold_distance):
                    ...
                    cv2.rectangle(frame, pt1=(int(nose_x - w / 2), int(nose_y - w / 2)),
                                  pt2=(int(nose_x + w / 2), int(nose_y + w / 2)), color=(255, 0, 0), thickness=2)
                    new_img = frame[int(nose_y - w / 2):int(nose_y + w / 2), int(nose_x - w / 2):int(nose_x + w / 2)]
                    image_counter += 1
                    cv2.imwrite(os.path.join(folder_pth, f'Expected_owner_{image_counter}.jpg'), new_img)
                else:
                    color = (0, 255, 0)
